...
> 状态：截至 2026-09-20 的已确认决定。与历史计划冲突时以本文件为准；未列为“已确认”的事项不得当作决定。

## Product direction

- 版本规则：所有功能全部完成前只发布 `0.0.x`；只有用户明确要求升级到 `0.1.0` 时才允许进入 `0.1.x`。
...
- 2026-09-15：普通行内和块级数学公式改用锁定版本的 RaTeX 解析、排版，由 GPUI 原生绘制字形和路径；Tectonic 仅处理 TikZ。公式不启动外部进程、不写磁盘缓存、不下载资源。
- 数学布局按源码和行内/块级样式在当前文档内共享，保留未改动公式，移除离开文档的公式；字号和颜色由绘制层处理。行内公式依据真实高度、深度与正文基线对齐，不固定图片高度。
- 公式兼容范围以 RaTeX 和本项目验收样例为准，不承诺完整 LaTeX 宏包或用户 TeX 前导内容；失败显示源码及局部错误，不自动回退 Tectonic。输入上限 16 KiB，并限制绘制复杂度和几何尺寸。
- 单独复制公式时才在内存生成透明 PNG，混合选择保留 LaTeX 文本。当前同一段落各折行共用最高公式的行高；逐行紧凑布局暂缓。
...
- macOS CI 生成原生 `.app` 和 DMG；`main` 更新滚动 nightly Release，版本标签生成正式 GitHub Release。开发阶段的 Homebrew Cask 从 nightly 安装。
- 手动安装的 `.app` 在 `⌘,` 设置中检查并安装经过内置公钥验证的更新。“之后自动更新并安装”必须由用户主动开启；关闭时不得后台联网检查。
- Apple Developer ID 签名与公证仍需相应凭据；在配置完成前，发布说明必须明确当前只有 ad-hoc 签名。
...
- 笔记内容始终在本地处理，不上传。
- 默认不上传遥测或诊断信息。
- 更新检查只访问 Rusidian 的 GitHub Release 元数据；自动检查默认关闭。
